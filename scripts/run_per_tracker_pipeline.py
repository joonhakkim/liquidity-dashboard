"""
코스피·코스닥 선행 PER 트래커 전용 경량 파이프라인. 장마감(15:30) 후 종가가 확정되는
매일 17:00에 실행되도록 별도 스케줄(register_per_tracker_task_windows.ps1)로 등록한다.
전체 run_pipeline.py(08:00/19:00, DART 스크리닝 포함이라 오래 걸림)와는 별개로,
이 트래커만 가볍게(각 50종목) 하루 한 번 더 갱신하기 위함이다.
"""
import subprocess
import sys
...
STEPS = [
    ("fetch_kospi_per_tracker.py", "코스피 선행 PER 트래커"),
    ("fetch_kosdaq_per_tracker.py", "코스닥 선행 PER 트래커"),
    ("build_per_tracker_page.py", "PER 트래커 페이지 빌드"),
...
def main():
    ...
    log_path = LOGS_DIR / f"{today}-per-tracker.log"
    ...
    with open(log_path, "a", encoding="utf-8") as log_f:
        ...
        try:
            ...
            if diff.returncode == 0:
                ...
                commit_msg = f"PER 트래커 자동 갱신 {today}"
        ...
        write(f"\n===== PER 트래커 파이프라인 실행 종료: {datetime.now().isoformat()} (성공={overall_ok}) =====")
