...
def build_secret_unlock_js(portfolio):
    """공개 페이지(트로이/모멘텀/민구/코스닥롱숏)의 비밀번호 입력창에서 마이 MP 전용
    비밀번호를 입력하면 my_mp.html로 바로 이동하는 JS 분기를 만든다(2026-09-15 사용자 요청 -
    ...
    if portfolio["id"] == "my_mp":
        return ""
    my_mp_cfg = next((p for p in PRIVATE_PORTFOLIOS if p["id"] == "my_mp"), None)
    if not my_mp_cfg:
        return ""
    return (
        ' else if (hash === "' + my_mp_cfg["pw_hash"] + '") {\n'
        '    sessionStorage.setItem("mp_unlocked_my_mp", "1");\n'
        '    window.location.href = "' + os.path.basename(my_mp_cfg["out_path"]) + '";\n'
        '  }'
    )
...
<body>
  ...
  <div class="note">
    ...
    차감합니다.<br><br>
    <b>연율화 표준편차</b> — 일별수익률의 표본표준편차 × &radic;252(연간 거래일수). MP 자체의
    변동성(위험)을 나타내는 가장 기본적인 지표입니다.<br><br>
    <b>정보비율(IR)</b> — (MP-BM 일별 초과수익의 평균 / 그 초과수익의 표준편차) × &radic;252.
    "벤치마크 대비 초과성과를 얼마나 꾸준하게(적은 변동으로) 냈는지"를 나타내며, 액티브 운용
    평가에서 샤프비율보다 더 자주 쓰이는 지표입니다. 값이 클수록 초과성과가 크고 안정적이라는
    뜻이고, 코스피/코스닥 각각 대비로 따로 계산합니다.
  </div>
...
<body>
  ...
  <div class="note">
    ...
    포지션임을 뜻합니다. 위 배지에 최신값이 표시됩니다(차트에는 MP·BM 지수만 그리고, 값 자체는
    이 배지로 확인). 단, 인버스ETF처럼 기초지수와 반대로 움직이는 상품은 "매수(BUY)"로 담겨
    있어도(공매도로 담으면 이중반전되어 버그가 됨) NET EXPOSURE 계산에서는 -1배로 보정해서 실제
    시장 방향성(코스닥 하락 베팅)이 정확히 반영되도록 합니다 - 매입금액/평가금액/수익률 자체는
    보정 없이 실제 매수 그대로입니다.<br><br>
    <b>MDD</b> — 편입 시작일 이후 지금까지 지수가 직전 최고점 대비 가장 많이 빠졌던 낙폭(%).<br><br>
    <b>연율화 표준편차</b> — 일별수익률의 표본표준편차 × &radic;252. MP 자체의 변동성(위험)을
    나타내는 가장 기본적인 지표입니다.<br><br>
    <b>소르티노비율</b> — 연율화 평균수익률 / 연율화 하방편차(손실 방향 변동성만 분모로 사용,
    최소수용수익률은 0으로 둠). 숏 포지션 때문에 상승장에서 변동성이 낮게 나오는 롱숏 구조의
    특성상, 상승/하락을 구분 안 하는 표준편차·샤프비율보다 "실제 손실 위험"을 더 정확히 반영하는
    지표라 롱숏 MP엔 이쪽을 씁니다.<br><br>
