"""
data/{market}_per_tracker.csv(선행/후행 PER 누적치) + 네이버 지수 시세를 묶어
docs/{market}_per_tracker.html(선행 PER 트래커) 를 만든다. 코스피/코스닥 공용.
"""
import json
import os
...
CHART_WINDOW_DAYS = 365  # 지수는 PER 수집 시작일과 무관하게 최근 1년치를 배경으로 깔아준다


def fetch_index_history(symbol, count=400):
    r = requests.get(
...
def build(market_key, market_label, index_symbol, out_name):
    per_path = os.path.join(DATA_DIR, f"{market_key}_per_tracker.csv")
    out_path = os.path.join(DOCS_DIR, out_name)

    per = pd.read_csv(per_path, parse_dates=["date"]).sort_values("date")
    idx = fetch_index_history(index_symbol).sort_values("date")

    cutoff = idx["date"].max() - pd.Timedelta(days=CHART_WINDOW_DAYS)
    idx_recent = idx[idx["date"] >= cutoff]

    merged = idx_recent.merge(per, on="date", how="left").sort_values("date")
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"저장 완료: {out_path} (지수 {len(merged)}일치, PER 수집 {len(per)}일치)")
...
<head>
<meta charset="utf-8">
<title>{market_label} 선행 PER 트래커</title>
...
<body>
  ...
  <a class="back" href="per_tracker.html">코스피</a>
  <a class="back" href="kosdaq_per_tracker.html">코스닥</a>
  <h1>{market_label} 선행 PER 트래커</h1>
  <div class="updated">최종 갱신: {updated_at} &middot; 최신 기준일 {latest_date} &middot; PER 수집 {n_days}일치(지수는 참고용으로 최근 1년 표시)</div>
...
new Chart(document.getElementById('perChart').getContext('2d'), {{
  ...
  data: {{
    labels: dates,
    datasets: [
      {{ label: '후행(TTM) PER', data: perTrailing, borderColor: '#adb5bd', backgroundColor: '#adb5bd', yAxisID: 'yPer', tension: 0.15, pointRadius: 4, pointBackgroundColor: '#adb5bd', pointBorderColor: '#adb5bd' }},
      {{ label: '당해선행(2026E) PER', data: per2026e, borderColor: '#ff6b6b', backgroundColor: '#ff6b6b', yAxisID: 'yPer', tension: 0.15, pointRadius: 4, pointBackgroundColor: '#ff6b6b', pointBorderColor: '#ff6b6b' }},
      {{ label: '차년선행(2027E) PER', data: per2027e, borderColor: '#4dabf7', backgroundColor: '#4dabf7', yAxisID: 'yPer', tension: 0.15, pointRadius: 4, pointBackgroundColor: '#4dabf7', pointBorderColor: '#4dabf7' }},
...
def main():
    build("kospi", "코스피", "KOSPI", "per_tracker.html")
    build("kosdaq", "코스닥", "KOSDAQ", "kosdaq_per_tracker.html")
