...
def build_merged():
    ...
    if not adr.empty and "kospi_adv" in adr.columns:
        ...
        # 거래일에만 행이 있어서(주말 없음) 여기서 거래일 기준으로 20개 롤링한 뒤 합친다 -
        # ffill된 캘린더일 그리드에서 rolling(20)하면 "20일"이 아니라 "20영업일+주말"이 섞여버림.
        adr = adr.sort_values("date").reset_index(drop=True)
        adr["kospi_adr"] = (
            adr["kospi_adv"].rolling(20, min_periods=20).sum()
            / adr["kospi_dec"].rolling(20, min_periods=20).sum() * 100
        )
        adr["kosdaq_adr"] = (
            adr["kosdaq_adv"].rolling(20, min_periods=20).sum()
            / adr["kosdaq_dec"].rolling(20, min_periods=20).sum() * 100
        )

    if not krx.empty and "kospi_close" in krx.columns:
        # 사용자가 보여준 리포트 차트의 "20일-100일 이격도"(단기 vs 중기 이평 괴리) 재현.
        # 거래일 기준 20일/100일 이동평균이라 krx(거래일에만 행 있음) 위에서 미리 계산한 뒤
        # 합친다 - ffill된 캘린더일 그리드에서 rolling하면 "20일"이 주말 낀 진짜 20일이
        # 아니라 20영업일+주말이 섞여버림(ADR과 동일한 이유).
        krx = krx.sort_values("date").reset_index(drop=True)
        krx["kospi_ma20"] = krx["kospi_close"].rolling(20, min_periods=20).mean()
        krx["kospi_ma100"] = krx["kospi_close"].rolling(100, min_periods=100).mean()
    ...
    # fred_raw.csv에 CBO 장기추정치(NROU, 테일러 준칙 트래커용, 2036년까지의 미래 전망치)가
    # 섞여 있어서 이 날짜들이 그대로 all_dates에 들어오면 대시보드 전체 날짜범위가 미래로
    # 10년치 늘어나버린다(2026-09-09, 사용자가 "유동성지표 차트가 안 보인다"고 알려줘서 발견 -
    # "5년" 버튼이 2031~2036의 텅 빈 미래 구간을 보여주고 있었음). 대시보드는 원래 전부
    # 과거~현재 데이터만 다루므로, 오늘 이후 날짜는 전부 잘라낸다.
    all_dates = all_dates[all_dates <= pd.Timestamp.now().normalize()]
    ...
    if "cma_balance" in merged.columns:
        ...
        merged["cma_balance_yoy"] = merged["cma_balance"].pct_change(252) * 100
        if "cma_balance" in raw_latest:
            raw_latest["cma_balance_qoq"] = raw_latest["cma_balance"]
            raw_latest["cma_balance_yoy"] = raw_latest["cma_balance"]
    ...
    if "broker_rp_balance" in merged.columns:
        merged["broker_rp_balance_qoq"] = merged["broker_rp_balance"].pct_change(63) * 100
        if "broker_rp_balance" in raw_latest:
            raw_latest["broker_rp_balance_qoq"] = raw_latest["broker_rp_balance"]
...
BULK_CANDIDATES = [
    ...
    ("cma_balance_qoq", "CMA잔고QoQ"), ("cma_balance_yoy", "CMA잔고YoY"), ("dry_powder_yoy", "실탄합계YoY"),
    ("broker_rp_balance_qoq", "대고객RP매도잔고QoQ"),
...
BULK_ALREADY_RATE_COLS = {
    ...
    "cma_balance_qoq", "cma_balance_yoy", "dry_powder_yoy", "broker_rp_balance_qoq",
}
...
def build_combined(merged, window_df, raw_latest):
    ...
    # 범례 자체를 큐레이션 목록(27개)만 남긴다 - 나머지 전체후보/이동평균은 계산은 여전히
...
def build_dashboard(merged, raw_latest):
    ...
    panels = {
        ...
        "코스피·코스닥 ADR": build_dual_panel(
            merged, recent,
            {"코스피 ADR(%)": "kospi_adr", "코스닥 ADR(%)": "kosdaq_adr"},
            {"코스피 종가": "kospi_close"},
        ),
        "코스피 이격도(20일-100일)": build_dual_panel(
            merged, recent,
...
<body>
  ...
  <div class="updated">최종 갱신일시: {updated_at} (기본은 최근 구간만 표시 - 아래 버튼이나 날짜 직접 지정으로 과거 구간도 조절 가능) &middot; <a href="index.html" style="color:#4dabf7;">&larr; 홈</a> &middot; <a href="op_band.html" style="color:#4dabf7;">OP밴드 트래커 &rarr;</a></div>
